...
cwd = os.getcwd()
dfwJsonFile = cwd + "/" + nsxmip +"_dfwrules_"+date +".json"
...
def send_request():
    try:
        ...
        dfwrules=response.text.encode('utf-8')
...
csvfilename = nsxmip + "_dfwrules_" + date +".csv"
...
for i in range(sectionCount):
        ...
        for h in range(ruleCount):
                ...
                try:
                        rulename = rule["name"].encode("utf-8")
                        rlist.append(rulename)
                except:
                        rlist.append("n/a")
                rlist.append(rule["sectionId"])
                rlist.append(rule["direction"].encode("utf-8"))
                ...
                if(rule.get("sources",0) > 0):
                        ...
                        # ruleSourceList is list
                        #print ruleSourceList
                        for s in range(len(ruleSourceList)):
                                ruleSourceCSVList = []
                                ruleSourceType = ruleSourceList[s]["type"].encode("utf-8")
                                ruleSourceValue = ruleSourceList[s]["value"].encode("utf-8")
                                try:
                                        ruleSourceName = ruleSourceList[s]["name"].encode("utf-8")
                                        ruleSourceCSVList.append(ruleSourceName.encode("utf-8"))
                                        ruleSourceCSVList.append(ruleSourceValue.encode("utf-8"))
                                        #ruleSourceCSVList.append(ruleSourceType.encode("utf-8"))
                                except:
